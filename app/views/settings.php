...
<main class="main narrow">
    <section class="hero hero-small"><div><p class="eyebrow">Configuration</p><h1>Settings</h1><p class="lede">Tune the local audit rules. The Kit API key is read from <code>.env</code> and is never stored in SQLite.</p></div></section>
    <section class="settings-card">
        <div class="settings-row"><div><span class="section-kicker">Kit connection</span><strong>API key status</strong><small>Use a Kit v4 API key from Developer settings.</small></div><span class="status-pill status-<?= $apiConfigured ? 'complete' : 'failed' ?>"><?= $apiConfigured ? 'Configured' : 'Missing' ?></span></div>
