...
Kit Subscriber Audit is designed for one trusted operator on a local machine. It is not a hosted SaaS application, an account-sharing tool, or a multi-tenant application. Subscriber emails, names, engagement history, and raw Kit responses are sensitive personal data.
...
Serve only `public/`. Keep full-disk encryption and secure backups enabled. The SQLite database is cleartext local data; encrypting the credentials separately does not encrypt subscriber data. Do not upload the database, `.env`, credential key, or logs to an issue, chat, cloud drive, or public repository.
